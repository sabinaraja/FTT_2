...
expName = 'FTT_2'  # from the Builder filename that created this script
expInfo = {
    '': '',
...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='/Users/sabina/Desktop/FTT_2/FTT_2_lastrun.py',
...
def run(expInfo, thisExp, win, inputs, globalClock=None, thisSession=None):
    ...
    fixationcross = visual.ImageStim(
        win=win,
        name='fixationcross', 
        image='Screen Shot 2024-04-08 at 3.33.49 PM.png', mask=None, anchor='center',
    ...
    MemorabilityFaces = visual.ImageStim(
        win=win,
        name='MemorabilityFaces', 
        image='default.png', mask=None, anchor='center',
        ori=0.0, pos=(0, 0), size=(0.5, 0.65),
        color=[1,1,1], colorSpace='rgb', opacity=None,
        flipHoriz=False, flipVert=False,
    ...
    text_11 = visual.TextStim(win=win, name='text_11',
        text='Press Y\nfor "yes"',
        font='Open Sans',
        pos=(-0.3, -0.2), height=0.05, wrapWidth=None, ori=0.0, 
        color='white', colorSpace='rgb', opacity=None, 
        languageStyle='LTR',
        depth=-1.0);
    ...
    continueRoutine = True
    # update component parameters for each repeat
    thisExp.addData('ProlificID.started', globalClock.getTime())
    textbox.reset()
    key_resp_6.keys = []
    key_resp_6.rt = []
    _key_resp_6_allKeys = []
    # keep track of which components have finished
    ProlificIDComponents = [text_14, textbox, key_resp_6]
    for thisComponent in ProlificIDComponents:
        thisComponent.tStart = None
        thisComponent.tStop = None
        thisComponent.tStartRefresh = None
        thisComponent.tStopRefresh = None
    ...
    while continueRoutine:
        ...
        keys = event.getKeys()  # get the pressed keys
        
        # Check if 'Enter' is pressed
        if 'return' in keys:
            continueRoutine = False  # End the routine and move to the next
        ...
        if key_resp_6.status == STARTED and not waitOnFlip:
            theseKeys = key_resp_6.getKeys(keyList=['enter', 'return'], ignoreKeys=["escape"], waitRelease=False)
    ...
    for thisComponent in ProlificIDComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    thisExp.addData('ProlificID.stopped', globalClock.getTime())
    ...
    Practice = data.TrialHandler(nReps=1.0, method='random', 
        ...
        trialList=data.importConditions('Face_trust_Practice_Stimuli.xlsx'),
    ...
    for thisPractice in Practice:
        ...
        # keep track of which components have finished
        fixationcross2Components = [fixationcross]
        ...
        while continueRoutine and routineTimer.getTime() < 0.5:
            ...
            if fixationcross.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > fixationcross.tStartRefresh + 0.5-frameTolerance:
                    # keep track of stop time/frame for later
        ...
        while continueRoutine and routineTimer.getTime() < 0.3:
            ...
            if practicefaces.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                ...
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'practicefaces.started')
                # update status
            ...
            if practicefaces.status == STARTED:
                ...
                if tThisFlipGlobal > practicefaces.tStartRefresh + 0.3-frameTolerance:
                    ...
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'practicefaces.stopped')
                    # update status
    ...
    trials = data.TrialHandler(nReps=1.0, method='random', 
        extraInfo=expInfo, originPath=-1,
        trialList=data.importConditions('face_trust2_Face_Stimuli.xlsx'),
    ...
    for thisTrial in trials:
        ...
        while continueRoutine and routineTimer.getTime() < 0.3:
            ...
            if FaceStimuli.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                ...
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'FaceStimuli.started')
                # update status
            ...
            if FaceStimuli.status == STARTED:
                ...
                if tThisFlipGlobal > FaceStimuli.tStartRefresh + 0.3-frameTolerance:
                    ...
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'FaceStimuli.stopped')
                    # update status
    ...
    continueRoutine = True
    # update component parameters for each repeat
    thisExp.addData('intructions_4.started', globalClock.getTime())
    # keep track of which components have finished
    intructions_4Components = [text_13]
    for thisComponent in intructions_4Components:
        thisComponent.tStart = None
        thisComponent.tStop = None
        thisComponent.tStartRefresh = None
        thisComponent.tStopRefresh = None
    ...
    for thisComponent in intructions_4Components:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    thisExp.addData('intructions_4.stopped', globalClock.getTime())
    # using non-slip timing so subtract the expected duration of this Routine (unless ended on request)
    if routineForceEnded:
        routineTimer.reset()
    else:
    ...
    gotValidClick = False  # until a click is received
    # keep track of which components have finished
    Instructions_3Components = [text_9, key_resp_4, mouse_2]
    ...
    trials_2 = data.TrialHandler(nReps=1.0, method='random', 
        extraInfo=expInfo, originPath=-1,
        trialList=data.importConditions('face_trust2_Memorability_Face_Stimuli.xlsx'),
    ...
    for thisTrial_2 in trials_2:
        ...
        continueRoutine = True
        # update component parameters for each repeat
        thisExp.addData('memorability_faces.started', globalClock.getTime())
        MemorabilityFaces.setImage(Memorability_Face_Stimuli)
        # keep track of which components have finished
        memorability_facesComponents = [MemorabilityFaces]
        for thisComponent in memorability_facesComponents:
            thisComponent.tStart = None
            thisComponent.tStop = None
            thisComponent.tStartRefresh = None
            thisComponent.tStopRefresh = None
        ...
        # --- Run Routine "memorability_faces" ---
        routineForceEnded = not continueRoutine
        while continueRoutine and routineTimer.getTime() < 0.3:
            # get current time
            t = routineTimer.getTime()
            ...
            # if MemorabilityFaces is starting this frame...
            if MemorabilityFaces.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                # keep track of start time/frame for later
                MemorabilityFaces.frameNStart = frameN  # exact frame index
                MemorabilityFaces.tStart = t  # local t and not account for scr refresh
                MemorabilityFaces.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(MemorabilityFaces, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'MemorabilityFaces.started')
                # update status
                MemorabilityFaces.status = STARTED
                MemorabilityFaces.setAutoDraw(True)
            
            # if MemorabilityFaces is active this frame...
            if MemorabilityFaces.status == STARTED:
                # update params
                pass
            
            # if MemorabilityFaces is stopping this frame...
            if MemorabilityFaces.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > MemorabilityFaces.tStartRefresh + 0.3-frameTolerance:
                    # keep track of stop time/frame for later
                    MemorabilityFaces.tStop = t  # not accounting for scr refresh
                    MemorabilityFaces.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'MemorabilityFaces.stopped')
                    # update status
                    MemorabilityFaces.status = FINISHED
        ...
        for thisComponent in memorability_facesComponents:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        thisExp.addData('memorability_faces.stopped', globalClock.getTime())
        # using non-slip timing so subtract the expected duration of this Routine (unless ended on request)
        if routineForceEnded:
            routineTimer.reset()
        else:
        ...
        continueRoutine = True
        # update component parameters for each repeat
        thisExp.addData('memorability.started', globalClock.getTime())
        key_resp_5.keys = []
        key_resp_5.rt = []
        _key_resp_5_allKeys = []
        # keep track of which components have finished
        memorabilityComponents = [text_10, text_11, text_12, key_resp_5]
        for thisComponent in memorabilityComponents:
            thisComponent.tStart = None
            thisComponent.tStop = None
            thisComponent.tStartRefresh = None
            thisComponent.tStopRefresh = None
        ...
        for thisComponent in memorabilityComponents:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        thisExp.addData('memorability.stopped', globalClock.getTime())
        # check responses
